...
public class DataController {
	...
	@PutMapping("/{cod}")
	public ResponseEntity<Object> update(@PathVariable(value = "cod") Long cod, @RequestBody @Valid DataDto dataDto) {
		try {
			return ResponseEntity.status(HttpStatus.OK).body(dataService.update(cod, dataDto));
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body(e.getMessage());
		}
	}
