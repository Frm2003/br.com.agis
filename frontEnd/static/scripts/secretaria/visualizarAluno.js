function selectByRa() {
    ...
    fetch(`http://localhost:8080/AGIS/matricula/aluno/${ra}`, { method: 'GET' })
        .then(response => { return response.json() })
        .then(data => { 
                ...
                if (matricula.situacao == 'dispensado') {
                    html += `
                        <div class="cardFooter" style="text-align: right; margin: .5em 0">
                            <button class="btForm" disabled>Dispensado</button>
                        </div>
                    </div>`
                } else {
                    html += `
                        <div class="cardFooter" style="text-align: right; margin: .5em 0">
                            <button class="btForm" onclick="dispensar(${matricula.aluno.ra}, ${matricula.turma.cod}, ${matricula.ano}, ${matricula.semestre})">Dispensar</button>
                        </div>
                    </div>`
                }
            })
